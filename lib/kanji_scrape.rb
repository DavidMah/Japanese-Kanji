...
def process_kanji(k, image_dir, data_dir, compound_dir)
  ...
  system_call("echo \"#{ROOT}/#{table}\" >> #{ROOT}/#{data_dir}/#{kanji_num}.html'")
...
level = ARGV[0] || 'grade2'
data_dir     = File.join(ROOT, "data#{level}")
image_dir    = File.join(ROOT, "image#{level}")
compound_dir = File.join(ROOT, "compound#{level}")
system("mkdir #{data_dir} && mkdir #{image_dir} && mkdir #{compound_dir}")
agent = Mechanize.new
agent.get("#{WEBSITE}blkod#{level}.htm")
